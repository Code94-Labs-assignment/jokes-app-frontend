...
const  getRandomJoke = async (type:string): Promise<AxiosResponse<any>> => {
  return await axiosDeliverJokesInstance.get(`/api/deliver-jokes/random?type=${type}`);
}
...
const Login = async (payload:loginPayload): Promise<AxiosResponse<any>> => {
  return await axiosModerateJokesInstance.post(`/api/moderate-jokes/auth/login`, payload);
}
...
const updateJoke = async (payload: any): Promise<AxiosResponse<any>> => {
  return await axiosModerateJokesInstance.put(`/api/moderate-jokes/${payload._id}`, payload);
}

const  approveJoke = async (id:string): Promise<AxiosResponse<any>> => {
  return await axiosModerateJokesInstance.post(`/api/moderate-jokes/approve/${id}`);
}

const rejectJoke = async (id:string): Promise<AxiosResponse<any>> => {
  return await axiosModerateJokesInstance.delete(`/api/moderate-jokes/reject/${id}`);
}
